...
class MCTS:
    ...
    def search(self, root_obs, root_history, goal: str, budget: int = 5, dpo_pairs: list[dict] = None, max_workers: int = 4):
        """
        Run MCTS search with parallel iterations.
        """
        logger.info(f"MCTS Search started | Budget: {budget} | Max workers: {max_workers} | Goal: {goal[:60]}... | Time: {datetime.now().strftime('%H:%M:%S')}")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i in range(budget):
                logger.info(f"MCTS | Submitting iteration {i+1}/{budget} to thread pool | Time: {datetime.now().strftime('%H:%M:%S')}")
                futures.append(executor.submit(self.run_iteration, root, goal, dpo_pairs, executor))
            ...
            for i, future in enumerate(futures, 1):
                try:
                    future.result()
                    completed += 1
                    logger.debug(f"MCTS | Iteration {i}/{budget} completed | Time: {datetime.now().strftime('%H:%M:%S')}")
                except Exception as e:
                    logger.error(f"MCTS | Iteration {i}/{budget} failed: {e} | Time: {datetime.now().strftime('%H:%M:%S')}", exc_info=True)
            
            logger.info(f"MCTS | All {completed}/{budget} iterations completed | Time: {datetime.now().strftime('%H:%M:%S')}")
        ...
        if not best_child and root.children:
            ...
            if valid_children:
                ...
                logger.warning(f"MCTS | No valid children, using fallback | Children: {len(root.children)}")
        
        if best_child:
            # Final validation of selected action
            if not best_child.action or not best_child.action.strip():
                logger.warning(f"MCTS | Selected child has empty action, returning None | Time: {datetime.now().strftime('%H:%M:%S')}")
                return None, root
            avg_value = best_child.value / best_child.visits if best_child.visits > 0 else 0
            logger.info(f"MCTS | Best action selected: {best_child.action} | Value: {avg_value:.3f} | Visits: {best_child.visits} | Time: {datetime.now().strftime('%H:%M:%S')}")
            return best_child.action, root
        
        logger.warning(f"MCTS | No best child found, returning None | Time: {datetime.now().strftime('%H:%M:%S')}")
        return None, root

    def run_iteration(self, root: MCTSNode, goal: str, dpo_pairs: list[dict], executor: ThreadPoolExecutor):
    ...
    def _compute_is_terminal(self, node: MCTSNode, goal: str) -> bool:
        ...
        judge = TerminalJudgePrompt(goal=goal, obs_summary=obs_prompt, screenshot=node.obs.get("screenshot"))
    ...
    def select(self, node: MCTSNode) -> MCTSNode:
        ...
        while curr.children:
            ...
            if unvisited:
                return max(unvisited, key=lambda c: c.q_mixed(self.alpha))
            
            # All children visited: use UCB1 with mixed Q
            curr = max(curr.children, key=lambda c: c.ucb1(alpha=self.alpha))
        return curr
    ...
    def expand(self, node: MCTSNode, goal: str, dpo_pairs: list[dict] = None):
        ...
        if node.obs is not None:
            ...
            logger.warning("expand() called on node with None obs and no parent obs, cannot rank actions")
        ...
        if isinstance(self.critic, TournamentCritic) and len(valid_actions) > 1:
            ...
            ranked = self.critic.rank_actions_tournament(goal, obs_to_use, valid_actions, self.obs_flags)
        else:
    ...
    def generate_candidate_actions(self, node: MCTSNode, goal: str, n=3, dpo_pairs: list[dict] = None) -> list[str]:
        ...
        obs_to_use = node.obs or (node.parent.obs if node.parent else None) or {
            "url": "", "axtree_txt": "No observation available", 
            "pruned_html": "", "dom_txt": "", "last_action_error": None, "focused_element_bid": None
        }
        ...
        dpo_context = ""
        if dpo_pairs:
            examples = [f"State: {p.get('state_summary', '')[:150]}...\n"
...
"""
        try:
            ...
            valid = [a.strip() for a in actions 
                    if a and a.strip() and '(' in a and ')' in a]
    ...
    def _simulate_with_fast_rewards(
        ...
        first_obs, first_reward = self._execute_single_action(parent_obs, action, goal)
        node.obs = first_obs
        ...
        for step in range(rollout_depth):
            if not current_node.children:
                obs_for_expand = current_node.obs or (current_node.parent.obs if current_node.parent else None)
                if obs_for_expand:
    ...
    def _simulate_with_real_rollouts(
        ...
        def critique_action(goal: str, obs_summary: str, action: str, obs: dict, pre_summary: str, error: str) -> dict:
            if self.use_env_reward:
    ...
    def _execute_single_action(self, start_obs: dict, action: str, goal: str) -> tuple[dict, float]:
        """
        ...
        if self.use_env_reward:
            ...
            return new_obs, float(env_reward >= 1.0)
        else:
            ...
            return new_obs, critique_result.get('score', 0.5)
    ...
    def critique(self, goal, obs_summary, action, obs=None, pre_obs_summary=None, action_error=None):
        """
        Use the modular critic to evaluate.
        """
        try:
            score = self.critic.evaluate(goal, action, obs, self.obs_flags, pre_obs_summary, action_error)
