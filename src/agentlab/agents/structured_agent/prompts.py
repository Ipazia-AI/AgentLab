...
Note: the element bids in these actions are historical and may no longer be valid on the current page.
...
## IMPORTANT PRUNING RULES:
    ...
    - Pruning is not always necessary. Just fill the field with an empty list if no changes are needed. Be precise in the changes you make, delete only nodes that do not lead to any further progress in the task.
