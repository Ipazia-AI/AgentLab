...
"""
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.Tab("HPA Plan") as tab_hpa_plan:
                hpa_plan_md = gr.Markdown()
        ...
        step_id.change(
            fn=update_step_info, outputs=[episode_info, action_info, state_error, hpa_plan_md]
        )
        ...
        tab_hpa_plan.select(fn=update_hpa_plan, outputs=hpa_plan_md)
...
def _format_hpa_plan_markdown(plan: dict) -> str:
    ...
    if not isinstance(tree_update, dict):
        ...
        if isinstance(pruned_nodes, list) and pruned_nodes:
            ...
            lines.append(code(str(pruned_nodes)))
        ...
        if isinstance(updated_nodes, dict) and updated_nodes:
            ...
            lines.append(code(str(updated_nodes)))
...
def update_hpa_plan():
    global info
    try:
        agent_info = info.exp_result.steps_info[info.step].agent_info or {}
        plan = agent_info.get("extra_info", {}).get("hpa_plan")
        if not plan:
            return "No HPA plan data available for this step."
        return _format_hpa_plan_markdown(plan)
    except (FileNotFoundError, IndexError):
        return None
...
def update_step_info():
    global info
    return [get_episode_info(info), get_action_info(info), get_state_error(info), update_hpa_plan()]
